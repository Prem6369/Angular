...
import { GuestPost } from '../../../../core/model/GuestDetails/guestDetails';
...
export class BookingPreviewComponent implements OnInit {
  ...
  initializer() {
    ...
    this.bookedRoomsArray = Object.entries(this.bookedRooms).map(
      ([key, value]) => ({
    ...
    this.roomTypes_Req = Object.entries(this.bookedRooms).map(
      ([key, value]) => ({
        room_type_id: value.room_type_id,
        room_type_count: value.count,
      })
    );
    this.check_in_date = this.toCustomFormat(
      this.booking_details.check_in_date
    );
    this.check_out_date = this.toCustomFormat(
      this.booking_details.check_out_date
    );
  }
