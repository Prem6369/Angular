...
export class ResortAddGuestComponent implements OnInit {
  ...
  removeExistGuest(id: number) {
    const index = this.guest.findIndex(
      (member: any) => member.guest_user_id === id
    );
    this.guest.splice(index, 1);
  }

  getValuesById(guest_user_id: number): GuestDetails[] {
    const elementIndex: number = this.guest.findIndex(
      (member: any) => member.guest_user_id === guest_user_id
    );
    if (elementIndex !== -1) {
  ...
  setType() {
    if (this.guest_user_id !== null && this.guest_user_id !== undefined) {
      this.Addguest.controls['guest_user_id'].setValue(
        this.getRandomInt(1000, 9999)
      );
      this.Addguest.controls['referrer_user_id'].setValue(
