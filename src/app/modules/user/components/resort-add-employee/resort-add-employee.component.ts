...
export class ResortAddEmployeeComponent implements OnInit {
  ...
  public _filter(value: any): UserProfile[] {
    ...
    return this.userProfile.filter(
      (option) =>
  ...
  removeEmployee(user_id: number) {
    const removeIndex: number = this.employee.findIndex(
      (emp: UserProfile) => emp.user_id === user_id
    );
    if (removeIndex !== -1) {
  ...
  setValues(user_id: number) {
    if (this.userProfile.length > 0) {
      const firstUserProfile = this.userProfile.find(
        (profile) => profile.user_id === user_id
      );
      if (firstUserProfile) {
        this.employees.controls['user_id'].setValue(user_id);
        this.employees.controls['phone_number'].setValue(
          firstUserProfile.phone_number
        );
      }
    }
  }
  ...
  getUsers() {
    this.repository.getAllUsers().subscribe((response: UserProfile[]) => {
