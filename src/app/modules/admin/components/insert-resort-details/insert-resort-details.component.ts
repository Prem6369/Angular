...
export class InsertResortDetailsComponent implements OnInit {
  ...
  getResortDetails() {
    ...
    this.approver_repo.getResortDetails(decrptyId).subscribe((response) => {
  ...
  selectRoomType(event: any) {
    ...
    const selectedRoom = this.roomsTypes.find(
      (room: { room_type_id: number }) => room.room_type_id === +roomTypeId
    );
    if (selectedRoom) {
  ...
  saveRooms() {
    if (this.AddRooms.value.number_of_rooms !== null) {
      const existingRoom = this.resort_rooms.find(
        (room) => room.room_type_id === this.AddRooms.value.room_type_id
      );
      if (existingRoom) {
    ...
    if (this.AddResort.valid) {
      ...
      const resort_details = {
        resort_id: this.Resort_id
          ? Number(atob(this.Resort_id.toString())) || 0
          : 0,
